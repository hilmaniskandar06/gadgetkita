      ...
      <div className="grid md:grid-cols-[340px_1fr] gap-6">
        <div className="bg-white border border-gray-200 rounded-xl p-5 h-fit">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Nama (Bank/E-Wallet)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Nomor Rekening/Akun</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Atas Nama (A/N)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Logo (Opsional)</label>
              <div className="flex items-center gap-3">
                {form.logo && <img src={form.logo} alt="Logo" className="w-10 h-10 object-contain bg-gray-50 rounded" />}
                <button
                  type="button"
                  disabled={!!uploadingField}
                  onClick={() => logoInputRef.current.click()}
                  className="flex items-center gap-1 text-xs border rounded px-2 py-1 hover:bg-gray-50 disabled:opacity-50"
                >
            ...
            {form.type === 'ewallet' && (
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">QRIS Code (Wajib untuk E-Wallet)</label>
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs border rounded px-2 py-1 hover:bg-gray-50 w-fit disabled:opacity-50"
                  >
            ...
            <div className="flex gap-2 mt-2">
              <button
                ...
                className="flex-1 bg-lime-500 hover:bg-lime-400 font-bold py-2 rounded-lg text-sm transition-colors disabled:opacity-50"
              >
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-x-auto">
          <table className="w-full text-sm min-w-[500px]">
            ...
            <tbody>
              {payments.map(p => (
                <tr key={p.id} className="border-t border-gray-100">
                  ...
                  <td className="px-4 py-3">
                    <div className="font-mono text-slate-600 font-bold">{p.account}</div>
                    <div className="text-xs text-slate-500">A/N: {p.accountName}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => handleEdit(p)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-gray-100"><Pencil size={14} /></button>
              ...
              {payments.length === 0 && <tr><td colSpan={4} className="px-4 py-10 text-center text-slate-500">Belum ada metode pembayaran</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
