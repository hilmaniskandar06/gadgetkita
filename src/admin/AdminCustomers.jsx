    ...
    <AdminShell title="Kelola Pelanggan">
      ...
      <div className="bg-white border border-cream-300 rounded-xl overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-cacao-500">Memuat data...</td></tr>
              ...
              <tr key={c.id} className="border-t border-cream-200">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-cacao-900">{c.name || 'User'}</div>
                      <div className="text-xs text-cacao-500 font-mono">{c.id.split('-')[0]}...</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-cacao-500 capitalize">{c.role}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 bg-cacao-900/50 flex items-center justify-center px-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <p className="text-xs text-cacao-500 text-center">Password dan Email hanya bisa diubah oleh pelanggan itu sendiri demi keamanan.</p>
        ...
        <div className="fixed inset-0 z-50 bg-cacao-900/50 flex items-center justify-center px-4" onClick={() => setShowDetail(null)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                <div className="text-xs font-semibold text-cacao-500">Email</div>
                <div>{showDetail.email}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-cacao-500">Telepon</div>
